...
// startedApp holds the AkashApp instance created for the running `start` command.
// It is captured here so the start command's PostSetup hook (which starts the eth
// JSON-RPC server) can access the app's RegisterPendingTxListener. Only the node
// process ever runs `start`, so a single package-level reference is sufficient.
var startedApp *akash.AkashApp
