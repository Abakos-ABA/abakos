...
type AkashApp struct {
	...
	*apptypes.App
	...
	// pendingTxListeners are notified (by the EVM ante handler) when an Ethereum
	// transaction enters the mempool; used by the JSON-RPC server for pending-tx
	// subscriptions.
...
func NewApp(
	...
	app.MM.SetOrderPreBlockers(
		...
		// EVM PreBlocker re-seals the process-global EVM coin config (uaba/6/aaba)
		// from on-chain state on every block; required after a node restart.
		evmtypes.ModuleName,
	)
	...
	evmAnteOptions := evmante.HandlerOptions{
		...
		EvmKeeper:              app.Keepers.EVM.EVM,
		FeegrantKeeper:         app.Keepers.Cosmos.FeeGrant,
		IBCKeeper:              app.Keepers.Cosmos.IBC,
		FeeMarketKeeper:        app.Keepers.EVM.FeeMarket,
	...
	// Abakos ante = EVM ante (routes eth vs cosmos) wrapped with our dynamic
	// minimum validator self-delegation check.
	anteHandler, err := NewAnteHandler(evmAnteOptions, app.Keepers.Cosmos.Bank, "uaba")
	if err != nil {
		panic(err)
	}
...
func orderBeginBlockers(modules []string) []string {
	...
	// Cosmos EVM: the EVM BeginBlocker must come after the FeeMarket BeginBlocker
	// (it consumes the base fee computed by feemarket).
	ord.Before(feemarkettypes.ModuleName, evmtypes.ModuleName)
...
// onPendingTx notifies registered listeners that an Ethereum transaction entered
// the mempool. It is wired into the EVM ante handler.
func (app *AkashApp) onPendingTx(hash ethcommon.Hash) {
	for _, listener := range app.pendingTxListeners {
		listener(hash)
	}
}

// RegisterPendingTxListener is used by the JSON-RPC server to subscribe to
// pending Ethereum transactions.
func (app *AkashApp) RegisterPendingTxListener(listener func(ethcommon.Hash)) {
