...
const (
	AccountAddressPrefix = "abakos"

	// AbakosEVMChainID is the EIP-155 chain id for the Abakos EVM. It is fixed
	// for the chain (MetaMask + eth tooling must use this id) and is independent
	// of the Cosmos chain-id string (abakos-sandbox-1).
	AbakosEVMChainID uint64 = 9721
)
...
type AppKeepers struct {
	...
	// Cosmos EVM keepers. Construction order at init: FeeMarket -> PreciseBank ->
	// EVM (with static precompiles) -> Erc20. PreciseBank bridges the 6-decimal
	// uaba bank denom to the 18-decimal representation the EVM expects.
	EVM struct {
...
func (app *App) InitNormalKeepers(
	...
	app.Keepers.EVM.FeeMarket = feemarketkeeper.NewKeeper(
		cdc,
		evmAuthority,
	...
	app.Keepers.EVM.PreciseBank = precisebankkeeper.NewKeeper(
		cdc,
	...
	app.Keepers.EVM.EVM = evmkeeper.NewKeeper(
		cdc,
		...
		app.Keepers.Cosmos.Staking,
		app.Keepers.EVM.FeeMarket,
		app.Keepers.Cosmos.ConsensusParams,
		&app.Keepers.EVM.Erc20,
		AbakosEVMChainID,
		"", // tracer: none by default
	).WithStaticPrecompiles(
		abakosStaticPrecompiles(app, cdc),
	)

	app.Keepers.EVM.Erc20 = erc20keeper.NewKeeper(
...
// mirrors cosmos/evm's DefaultStaticPrecompiles but omits the ICS20 precompile
// (which requires the cosmos/evm IBC transfer keeper); the IBC<->ERC20 bridge is
// out of scope for v1. The &Erc20 pointer is stable and is populated right after
// the EVM keeper is built.
func abakosStaticPrecompiles(app *App, cdc codec.Codec) map[ethcommon.Address]ethvm.PrecompiledContract {
	precompiles := precompiletypes.NewStaticPrecompiles().
		WithPraguePrecompiles().
		WithP256Precompile().
		WithBech32Precompile().
		WithStakingPrecompile(*app.Keepers.Cosmos.Staking, app.Keepers.EVM.PreciseBank).
		WithDistributionPrecompile(app.Keepers.Cosmos.Distr, *app.Keepers.Cosmos.Staking, app.Keepers.EVM.PreciseBank).
		WithBankPrecompile(app.Keepers.EVM.PreciseBank, &app.Keepers.EVM.Erc20).
		WithGovPrecompile(*app.Keepers.Cosmos.Gov, app.Keepers.EVM.PreciseBank, cdc).
		WithSlashingPrecompile(app.Keepers.Cosmos.Slashing, app.Keepers.EVM.PreciseBank)

	return map[ethcommon.Address]ethvm.PrecompiledContract(precompiles)
}
